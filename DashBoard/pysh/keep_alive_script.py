...
class AIHealthKeepAlive:
    def __init__(self, websocket_url, ota_url, device_id="web_keep_alive_client", client_id="keep_alive_client", device_mac="00:11:22:33:44:55"):
        ...
        self.keep_alive_interval = 15  # 15秒间隔
        self.ota_keep_alive_interval = 30  # OTA保活间隔30秒
        self.test_message_interval = 60  # 测试消息间隔60秒
        self.max_retries = 3
        self.retry_delay = 5
        self.session = None
    ...
    async def send_keep_alive_message(self):
        ...
        try:
            ...
            keep_alive_message = {
                ...
                "mode": "manual",
                "state": "detect",
                "text": "Dialogue test instruction (single sentence): Please reply with only the number '1' and must not contain any other characters, spaces, line breaks, or explanations"  # 简单的ping消息
            }
    ...
    async def send_test_message(self):
        ...
        try:
            ...
            test_message = {
                ...
                "mode": "manual",
                "state": "detect",
                "text": "请简单介绍一下你自己"  # 这个消息应该能够触发大模型回复
            }
    ...
    async def run_keep_alive(self):
        ...
        while retry_count < self.max_retries:
            try:
                ...
                # 保活消息循环
                websocket_last_keep_alive = time.time()
                ota_last_keep_alive = time.time()
                test_last_message = time.time()
...
async def main():
    # 配置参数 - 根据实际情况修改
    WEBSOCKET_URL = "ws://10.0.0.1:8000/xiaozhi/v1/"
    OTA_URL = "http://10.0.0.1:8003/xiaozhi/ota/"
