...
class Assistant:
    ...
    def display_sound_energy(self, energy):
        ...
        for i in range(-int(np.floor(COL_COUNT/2)), int(np.ceil(COL_COUNT/2))):
            ...
            for i in range(0, mid):
                color = (255, 0, 0) if i < RED_CENTER else (0, 255, 0)
    ...
    def ask_ollama(self, prompt, responseCallback):
        ...
        responseCallback(output['message']['content'])
    ...
    def text_to_speech(self, text):
        ...
        stream = self.audio.open(format =
                        ...
                        output = True) # 打开音频流
        chunkSize = 1024
        chunk = wf.readframes(chunkSize) # 读取数据
        while chunk:
            stream.write(chunk)
