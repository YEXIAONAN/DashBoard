...
async def chat_with_dify(text: str, user_name: str = "用户", language: str = "zh") -> str:
    ...
    注意：优先使用 Ollama 处理越南语，如果失败则尝试 Dify
    """
    ...
    if language == "vi":
        ...
        try:
            ...
            logger.info("尝试使用 Dify 处理越南语（可能效果不佳）")
            # 继续执行 Dify 调用
...
async def chat_with_ollama_fallback(text: str, language: str = "zh") -> str:
    ...
    try:
        logger.info(f"发送到 Ollama: {text[:50]}... (语言: {language})")
        ...
        error_msg = f"无法连接到 Ollama 服务 ({OLLAMA_HOST})"
        logger.error(f"{error_msg}: {str(e)}")
        raise Exception(error_msg)
    except httpx.TimeoutException as e:
        error_msg = "Ollama 服务响应超时"
        logger.error(f"{error_msg}: {str(e)}")
        raise Exception(error_msg)
    except Exception as e:
        error_msg = f"Ollama 服务错误: {str(e)}"
